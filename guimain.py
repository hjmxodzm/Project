...
class SSHGui:
    def __init__(self, master):
        ...
        ssh_frame.grid(row=1, column=0, columnspan=2,
                       padx=5, pady=5, sticky='w')

        ttk.Label(ssh_frame, text='IP 주소').grid(
            row=0, column=0, padx=5, pady=5, sticky='w')
        ttk.Label(ssh_frame, text='포트 번호').grid(
            row=1, column=0, padx=5, pady=5, sticky='w')
        ttk.Label(ssh_frame, text='사용자 이름').grid(
            row=2, column=0, padx=5, pady=5, sticky='w')
        ttk.Label(ssh_frame, text='SSH 비밀번호').grid(
            row=3, column=0, padx=5, pady=5, sticky='w')
        ttk.Label(ssh_frame, text='MySQL 비밀번호').grid(
        ...
        self.mysqlPassword_entry = ttk.Entry(
            ssh_frame, show='*', state='disabled')
        self.mysqlPassword_entry.grid(
            row=4, column=1, padx=5, pady=5, sticky='w')

        # 체크박스
        checkbox_frame = ttk.LabelFrame(main_frame, text='진단 항목', padding=10)
        checkbox_frame.grid(row=1, column=1, columnspan=2,
                            padx=5, pady=5, sticky='w')

        self.check_openstack_var = tk.BooleanVar(value=False)
        self.check_openstack = ttk.Checkbutton(
            checkbox_frame, text='OpenStack', variable=self.check_openstack_var)
        self.check_openstack.grid(row=0, column=0, padx=5, pady=5, sticky='w')

        self.check_linux_var = tk.BooleanVar(value=False)
        self.check_linux = ttk.Checkbutton(
            checkbox_frame, text='Linux', variable=self.check_linux_var)
        self.check_linux.grid(row=0, column=1, padx=5, pady=5, sticky='w')

        self.check_apache_var = tk.BooleanVar(value=False)
        self.check_apache = ttk.Checkbutton(
            checkbox_frame, text='Apache', variable=self.check_apache_var)
        self.check_apache.grid(row=1, column=0, padx=5, pady=5, sticky='w')

        self.check_docker_var = tk.BooleanVar(value=False)
        self.check_docker = ttk.Checkbutton(
            checkbox_frame, text='Docker', variable=self.check_docker_var)
        self.check_docker.grid(row=1, column=1, padx=5, pady=5, sticky='w')

        self.check_mysql_var = tk.BooleanVar(value=False)
        self.check_mysql = ttk.Checkbutton(
            checkbox_frame, text='MySQL', variable=self.check_mysql_var, command=self.on_mysql_checkbox_change)
        self.check_mysql.grid(row=2, column=0, padx=5, pady=5, sticky='w')

        # 실행 버튼
        self.execute_btn = ttk.Button(
            main_frame, text='진단 실행', command=self.execute)
        ...
        self.folder_btn = ttk.Button(
            main_frame, text='보고서 열기', command=self.folder)
        ...
        report_frame = ttk.LabelFrame(
            main_frame, text='Made by 정보는 역시 심정보', padding=10)
        report_frame.grid(row=4, column=0, columnspan=2,
                          padx=5, pady=5, sticky='w')
            ...
            try:
                ...
                if self.check_linux_var.get():
                    self.output_text.insert(
                        tk.END, f"linux report downloading...\n")
                    self.output_text.see(tk.END)
                    src1 = './script/linux_report.csv'
                    dest1 = f'./backend/vulscan_csv/linux_report.csv'
                    sftp.get(src1, dest1)
                    subprocess.run(
                        ['python', './backend/linux/linux_report.py'])
                    subprocess.run(
                        ['python', './backend/linux/linux_graph.py'])
                    subprocess.run(
                        ['python', './backend/linux/linux_result.py'])
                if self.check_apache_var.get():
                    self.output_text.insert(
                        tk.END, f"apache_report downloading...\n")
                    self.output_text.see(tk.END)
                    src2 = './script/apache_report.csv'
                    dest2 = f'./backend/vulscan_csv/apache_report.csv'
                    sftp.get(src2, dest2)
                    subprocess.run(
                        ['python', './backend/apache/apache_report.py'])
                    subprocess.run(
                        ['python', './backend/apache/apache_graph.py'])
                    subprocess.run(
                        ['python', './backend/apache/apache_result.py'])
                if self.check_docker_var.get():
                    self.output_text.insert(
                        tk.END, f"docker_report downloading...\n")
                    self.output_text.see(tk.END)
                    src3 = './script/docker_report.csv'
                    dest3 = f'./backend/vulscan_csv/docker_report.csv'
                    sftp.get(src3, dest3)
                    subprocess.run(
                        ['python', './backend/docker/docker_report.py'])
                    subprocess.run(
                        ['python', './backend/docker/docker_graph.py'])
                    subprocess.run(
                        ['python', './backend/docker/docker_result.py'])
                if self.check_mysql_var.get():
                    self.output_text.insert(
                        tk.END, f"mysql_report downloading...\n")
                    self.output_text.see(tk.END)
                    src4 = './script/my-sql_report.csv'
                    dest4 = f'./backend/vulscan_csv/mysql_report.csv'
                    sftp.get(src4, dest4)
                    subprocess.run(
                        ['python', './backend/mysql/mysql_report.py'])
                    subprocess.run(
                        ['python', './backend/mysql/mysql_graph.py'])
                    subprocess.run(
                        ['python', './backend/mysql/mysql_result.py'])
                if self.check_openstack_var.get():
                    self.output_text.insert(
                        tk.END, f"openstack_report downloading...\n")
                    self.output_text.see(tk.END)
                    src4 = './script/openstack_report.csv'
                    dest4 = f'./backend/vulscan_csv/openstack_report.csv'
                    sftp.get(src4, dest4)
                    subprocess.run(
                        ['python', './backend/openstack/openstack_report.py'])
                    subprocess.run(
                        ['python', './backend/openstack/openstack_graph.py'])
                    subprocess.run(
                        ['python', './backend/openstack/openstack_result.py'])
                sftp.close()
            except:
